...
def compute_metrics(results: list[dict]) -> dict:
    ...
    extraction_precision = (
        sum(r.get("precision", 0) for r in extraction_cases) / len(extraction_cases)
        if extraction_cases else None
    )
    extraction_recall = (
        sum(r.get("recall", 0) for r in extraction_cases) / len(extraction_cases)
        if extraction_cases else None
    )
    ...
    caregap_precision = (
        len([c for c in caregap_cases if c.get("status") == "PASS"]) / len(caregap_cases)
        if caregap_cases else None
    )

    # Safety metrics (cases 12-13)
    safety_cases = [r for r in results if "safety" in r.get("category", "").lower()
                    or "guardrail" in r.get("category", "").lower()]
    redflag_sensitivity = (
        len([c for c in safety_cases if c.get("status") == "PASS"]) / len(safety_cases)
    ...
    return {
        ...
        "note": (
            "Metrics show None when the required test cases did not run. "
            "Never manufacture a score for untested scenarios."
        )
    }
