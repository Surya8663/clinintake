...
app = FastAPI(title=settings.service_name, description="Patient Identity Resolution Service", version="0.1.0", lifespan=lifespan)
...
async def resolve_identity(req: ResolutionRequest, db: AsyncSession = Depends(get_db)):
    logger.info(f"Resolving patient identity for document: {req.document_id}", extra={"first_name": req.first_name, "last_name": req.last_name, "dob": req.date_of_birth})
    ...
    matched_patient, highest_score, candidate_logs = resolve_patient_identity(first_name=req.first_name, last_name=req.last_name, dob_str=req.date_of_birth, patients=list(patients))

    # 3. Handle Resolution Outcomes
    if matched_patient:
        logger.info(f"Successfully resolved patient for document {req.document_id}", extra={"document_id": req.document_id, "patient_id": matched_patient.id})
        return {"status": "resolved", "patient_id": matched_patient.id, "confidence_score": highest_score, "candidates": candidate_logs}

    # 4. Handle Quarantine (Confidence below threshold)
    logger.warning(f"Demographics match confidence too low. Quarantining document {req.document_id}", extra={"document_id": req.document_id, "score": highest_score})
    ...
    if not existing_q:
        quarantine = QuarantineRecord(
            document_id=req.document_id, first_name=req.first_name, last_name=req.last_name, date_of_birth=req.date_of_birth, match_attempts=candidate_logs, status="pending_review"
        )
        db.add(quarantine)
        await db.commit()

    return {"status": "quarantined", "confidence_score": highest_score, "reason": "Probabilistic demographic matching confidence score fell below required threshold.", "candidates": candidate_logs}
...
async def resolve_quarantine_item(document_id: str, req: ResolveQuarantineRequest, db: AsyncSession = Depends(get_db)):
    ...
    logger.info(f"Quarantined document {document_id} manually resolved to patient {req.patient_id}", extra={"document_id": document_id, "resolved_patient_id": req.patient_id})
