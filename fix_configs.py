...
def process_file(filepath):
    ...
    content = re.sub(r'(:\s*str\s*=\s*Field\(\s*)default="http://localhost:[^"]+"(\s*\))', r"\1...\2", content)
    # Also if there are description fields, we just remove the default
    content = re.sub(r'default="http://localhost:[^"]+",', r"... ,", content)

    # For os.getenv("...", "http://localhost:...")
    content = re.sub(r'os\.getenv\("([^"]+)", "http://localhost:[^"]+"\)', r'os.getenv("\1")', content)

    # For CONFIDENCE_THRESHOLD
    content = re.sub(r"CONFIDENCE_THRESHOLD:\s*float\s*=\s*Field\(default=[0-9.]+\)", r"CONFIDENCE_THRESHOLD: float = Field(...)", content)
    content = re.sub(r"confidence_threshold:\s*float\s*=\s*Field\(default=[0-9.]+\)", r"confidence_threshold: float = Field(...)", content)
