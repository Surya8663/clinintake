...
app = FastAPI(title=settings.service_name, description="Deterministic Clinical Quality Language (CQL) Rules Engine", version="1.0.0")
...
async def evaluate_cql(request: CQLEvaluateRequest):
    ...
    response = evaluate_cql_rules(patient_id=request.patient_id, clinical_data=request.clinical_data, rule_libraries=request.rule_library or ["Diabetes_Screening", "Hypertension_Control"])
    return response
