...
app = FastAPI(title=settings.service_name, description="Compliance Dashboard REST API & UI querying audit-service API exclusively", version="2.0.0")
...
async def health_check():
    return {"status": "ok", "service": settings.service_name, "audit_service_url": settings.audit_service_url}
...
async def verify_compliance_vault(
    credentials: HTTPAuthorizationCredentials | None = Depends(security_bearer), claims: dict[str, Any] = Depends(require_roles(["compliance:audit:read", "admin:system"]))
):
