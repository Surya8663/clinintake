...
_PHI_PATTERN = re.compile(r"\b(patient_name|date_of_birth|dob|ssn|address|clinical_note)\b", re.IGNORECASE)
...
def _redact_dict(record: dict) -> dict:
    ...
    for key, value in record.items():
        if key.lower() in _PHI_FIELD_NAMES:
            ...
            sanitized[key] = [_redact_dict(v) if isinstance(v, dict) else v for v in value]
...
def configure_phi_safe_logging(service_name: str = "clinintake-service", level: str = "INFO") -> None:
    ...
    logging.getLogger(service_name).info("PHI-safe structured JSON logging configured.", extra={"service_name": service_name})
