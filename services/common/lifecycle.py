    ...
    try:
        async with httpx.AsyncClient(timeout=timeout_s) as client:
            ...
            if resp.status_code >= 500:
                raise StartupDependencyError(f"Service '{service_label}' at {url} returned HTTP {resp.status_code}.")
            logger.info(f"Startup: dependency '{service_label}' is reachable (HTTP {resp.status_code}).")
    except httpx.ConnectError as exc:
        raise StartupDependencyError(f"Startup: required dependency '{service_label}' is unreachable at {url}: {exc}") from exc
    except httpx.TimeoutException as exc:
        raise StartupDependencyError(f"Startup: required dependency '{service_label}' timed out at {url}: {exc}") from exc
...
async def validate_required_env_vars(*var_names: str) -> None:
    ...
    if missing:
        raise StartupDependencyError(f"FATAL: Missing required environment variables: {', '.join(missing)}. " f"Service cannot start safely without these values.")
