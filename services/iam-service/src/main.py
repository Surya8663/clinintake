...
app = FastAPI(title=settings.service_name, description="Keycloak OIDC Integration Facade and Token Verification Service", version="2.0.0")
...
async def health_check():
    return {"status": "ok", "service": settings.service_name, "oidc_realm": settings.keycloak_realm, "token_ttl_minutes": settings.access_token_expire_minutes}
...
async def login_oidc(request: LoginRequest):
    ...
    return TokenResponse(access_token=token, token_type="bearer", expires_in_seconds=expires_in_sec, role=role, scopes=scopes)
...
async def get_m2m_token(request: M2MTokenRequest):
    ...
    try:
        ...
        return TokenResponse(access_token=token, token_type="bearer", expires_in_seconds=expires_in_sec, role="CLINICAL_AGENT", scopes=["service:internal"])
