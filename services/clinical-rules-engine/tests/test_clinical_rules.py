...
def test_cql_evaluation_diabetes_inclusion():
    clinical_data = {
        "diagnoses": [
            {"name": {"value": "Essential Hypertension"}, "icd10_code": {"value": "I10"}}
        ],
        "medications": [
            {"name": {"value": "Lisinopril 10mg"}, "rxnorm_code": {"value": "314076"}}
        ],
        "labs": [
            {"name": {"value": "HbA1c"}, "loinc_code": {"value": "4548-4"}, "value": {"value": "7.2 %"}}
        ]
    }
    ...
    response = client.post(
        ...
        json={
            ...
            "rule_library": ["Diabetes_Screening", "Hypertension_Control"]
        }
    )
    assert response.status_code == 200
    data = response.json()
