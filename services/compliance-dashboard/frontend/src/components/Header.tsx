...
import { Shield, Activity, UserCheck, Stethoscope, BarChart3 } from 'lucide-react';

interface HeaderProps {
  serviceStatus: 'connected' | 'error' | 'loading';
}

export const Header: React.FC<HeaderProps> = ({ serviceStatus }) => {
  return (
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-200/80 text-xs">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 px-3 py-1 rounded-lg bg-slate-50 border border-slate-200 text-[11px] font-medium text-slate-700">
            ...
            <strong className="text-blue-700 font-mono">Compliance Reviewer</strong>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-800">
            Compliance Audit Vault Dashboard
          </h1>
          <p className="text-xs text-slate-600 mt-0.5">
            Immutable Audit Trail &amp; Cryptographic Vault Integrity Verification
          </p>
        </div>
      </div>
    </header>
