...
def build_and_validate_fhir_resources(extraction_data: ExtractionData) -> list[dict[str, Any]]:
    ...
    if extraction_data.patient_id.value != "Incomplete":
        try:
            patient = Patient.model_validate({"resourceType": "Patient", "id": extraction_data.patient_id.value, "active": True})
                ...
                condition = Condition.model_validate(
                    {
                        ...
                        "code": {"coding": [{"system": "http://hl7.org/fhir/sid/icd-10-cm", "code": code_str, "display": diag.name.value}], "text": diag.name.literal_quote},
                        "subject": {"reference": f"Patient/{extraction_data.patient_id.value}"},
                    }
                )
                ...
                med_stmt = MedicationStatement.model_validate(
                    {
                        ...
                        "id": f"med-{idx+1}",
                        "status": "recorded",
                        "medication": {"concept": {"coding": [{"system": "http://www.nlm.nih.gov/research/umls/rxnorm", "code": rx_code, "display": med.name.value}], "text": med.name.literal_quote}},
                        "dosage": [{"text": med.dosage.value if med.dosage.value != "Incomplete" else "as directed"}],
                        "subject": {"reference": f"Patient/{extraction_data.patient_id.value}"},
                    }
                )
                ...
                obs = Observation.model_validate(
                    {
                        ...
                        "id": f"obs-{idx+1}",
                        "status": "final",
                        "code": {"coding": [{"system": "http://loinc.org", "code": loinc, "display": lab.name.value}], "text": lab.name.literal_quote},
