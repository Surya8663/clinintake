...
def detect_clinical_redflags(
    ...
    if qsofa_score is not None and qsofa_score >= 2:
        red_flags.append(RedFlagTrigger(
            syndrome="sepsis",
            severity="EMERGENCY",
            description=f"qSOFA score = {qsofa_score} (>= 2 indicates high risk of Sepsis mortality).",
            trigger_source="qSOFA_Criteria"
        ))
    elif news2_score is not None and news2_score >= 7:
        red_flags.append(RedFlagTrigger(
            syndrome="sepsis",
            severity="EMERGENCY",
            description=f"NEWS2 score = {news2_score} (>= 7 indicates high clinical risk / emergency response required).",
            trigger_source="NEWS2_Score"
        ))

    # 2. Vitals-driven Respiratory / Hypoxia Red Flags
    if vitals and vitals.spo2 is not None and vitals.spo2 < 90.0:
        red_flags.append(RedFlagTrigger(
            syndrome="severe_respiratory_distress",
            severity="EMERGENCY",
            description=f"Critical Hypoxia: SpO2 = {vitals.spo2}% (< 90%).",
            trigger_source="NEWS2_Score"
        ))
            ...
            if kw in text_content:
                ...
                if not any(rf.syndrome == category for rf in red_flags):
                    red_flags.append(RedFlagTrigger(
                        syndrome=category,
                        severity="EMERGENCY",
                        description=f"Clinical Red Flag: {desc} (Trigger: '{kw}').",
                        trigger_source="Heuristic_RedFlag_Keywords"
                    ))
                break
