...
from services.orchestrator.src.lyzr_client import (
    lyzr_client, LyzrSuperFlowClient, LyzrApiError, LyzrGovernanceViolationError
)
import importlib
...
def test_lyzr_superflow_execution_returns_ids_and_nodes():
    res = lyzr_client.start_superflow_execution(
        workflow_id="sf_clinintake_dag_v3_99",
        document_id="DOC-GOV-101",
        input_payload={"file_path": "uploads/doc101.pdf", "state": "received"}
    )
...
def test_prompt_injection_blocked_by_lyzr_policy():
    ...
    with pytest.raises((LyzrGovernanceViolationError, RuntimeError)) as exc_info:
        lyzr_client.start_superflow_execution(
            ...
            input_payload={"raw_text": malicious_text}
        )
...
def test_missing_lyzr_credentials_fails_honestly():
    ...
    with pytest.raises(LyzrApiError) as exc_info:
        custom_client.start_superflow_execution(
            workflow_id="sf_clinintake_dag_v3_99",
            document_id="DOC-NO-CRED",
            input_payload={"raw_text": "Normal text"}
        )
