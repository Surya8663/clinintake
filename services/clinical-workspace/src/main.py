...
app = FastAPI(title=settings.service_name, description="Clinical Workspace Reviewer API & Dashboard Backend", version="2.0.0")
...
def _ensure_doc_in_db(document_id: str):
    if document_id not in REVIEW_DATABASE:
        REVIEW_DATABASE[document_id] = {
            ...
            "evidence_spans": [{"field_name": "hbA1c", "source_quote": "HbA1c 8.2 %", "bbox": [10, 20, 100, 30]}],
...
async def stream_document_content(document_id: str, range: str | None = Header(None), claims: dict[str, Any] = Depends(require_roles(["clinician:review", "admin:system"]))):
    """
    ...
    headers = {"Accept-Ranges": "bytes", "Content-Length": str(total_bytes), "Content-Type": "application/pdf", "Content-Disposition": f'inline; filename="{document_id}.pdf"'}
...
async def list_review_queue(claims: dict[str, Any] = Depends(require_roles(["clinician:review", "admin:system"]))):
    ...
    return [ReviewItem(document_id=item["document_id"], patient_id=item["patient_id"], status=item["status"], created_at=item["created_at"]) for item in REVIEW_DATABASE.values()]


@app.get("/workspace/findings/{document_id}", response_model=DocumentFindingsResponse)
async def get_document_findings(document_id: str, claims: dict[str, Any] = Depends(require_roles(["clinician:review", "admin:system"]))):
    ...
    return DocumentFindingsResponse(
        document_id=rec["document_id"], patient_id=rec["patient_id"], referral_text=rec["referral_text"], evidence_spans=[EvidenceSpan(**ev) for ev in rec["evidence_spans"]], status=rec["status"]
    )


@app.put("/workspace/referral/{document_id}")
async def edit_referral_text(document_id: str, body: ReferralEditRequest, claims: dict[str, Any] = Depends(require_roles(["clinician:review", "admin:system"]))):
...
async def submit_clinician_decision(document_id: str, body: DecisionSubmitRequest, claims: dict[str, Any] = Depends(require_roles(["clinician:approve", "clinician:reject", "admin:system"]))):
    ...
    return DecisionSubmitResponse(
        document_id=document_id, decision=dec, status=new_status, signed_event_emitted=is_event_emitted, message=f"Clinician decision '{dec}' recorded for clinician {verified_clinician_id}."
    )
