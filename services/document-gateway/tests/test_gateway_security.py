...
def test_jwt_auth_missing_header():
    ...
    response = client.post("/gateway/upload", files={"file": ("doc.pdf", pdf_bytes, "application/pdf")})
    assert response.status_code == 422  # FastAPI validation error for missing header


def test_jwt_auth_invalid_token():
    pdf_bytes = generate_minimal_pdf("Healthy patient data")
    response = client.post("/gateway/upload", files={"file": ("doc.pdf", pdf_bytes, "application/pdf")}, headers={"Authorization": "Bearer bad-token-signature"})
...
def test_mime_spoofing_rejected(mock_post):
    ...
    mock_post.return_value = MagicMock(status_code=200, json=lambda: {"is_safe": False, "reason": "MIME check failed. Expected application/pdf"})

    response = client.post("/gateway/upload", files={"file": ("spoofed.pdf", bad_bytes, "application/pdf")}, headers=get_auth_headers())
...
def test_malware_file_rejected(mock_post):
    ...
    mock_post.return_value = MagicMock(status_code=200, json=lambda: {"is_safe": False, "reason": "Malware detected by ClamAV: Eicar-Test-Signature FOUND"})

    response = client.post("/gateway/upload", files={"file": ("infected.pdf", pdf_bytes, "application/pdf")}, headers=get_auth_headers())
    assert response.status_code == 400
...
def test_prompt_injection_file_rejected(mock_post):
    ...
    mock_post.return_value = MagicMock(status_code=200, json=lambda: {"is_safe": False, "reason": "Prompt injection payload matched"})

    response = client.post("/gateway/upload", files={"file": ("adversarial.pdf", adversarial_pdf, "application/pdf")}, headers=get_auth_headers())
    assert response.status_code == 400
    assert "Security violation detected" in response.json()["detail"]
...
def test_dmz_unreachable_fails_closed(mock_post):
    ...
    response = client.post("/gateway/upload", files={"file": ("clean.pdf", pdf_bytes, "application/pdf")}, headers=get_auth_headers())
...
def test_kms_disk_encryption(mock_post):
    ...
    mock_post.return_value = MagicMock(status_code=200, json=lambda: {"is_safe": True, "reason": "Passed safety checks"})

    response = client.post("/gateway/upload", files={"file": ("clean_intake.pdf", pdf_bytes, "application/pdf")}, headers=get_auth_headers())
