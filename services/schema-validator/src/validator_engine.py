...
FHIR_RESOURCE_MAP = {"Condition": Condition, "MedicationStatement": MedicationStatement, "Observation": Observation, "Patient": Patient, "Encounter": Encounter}
...
def validate_fhir_resource_schema(resource_type: str, fhir_resource: dict[str, Any]) -> ValidateSchemaResponse:
    ...
    if not isinstance(fhir_resource, dict):
        issues.append(ValidationIssue(field="root", issue_type="invalid_type", description="Payload must be a JSON object", severity="error"))
        return ValidateSchemaResponse(is_valid=False, resource_type=resource_type, issues=issues)
    ...
    try:
        ...
        return ValidateSchemaResponse(is_valid=True, resource_type=res_type, issues=[], validated_resource=validated_obj.model_dump(mode="json"))
        ...
        for err in val_err.errors():
            ...
            issues.append(ValidationIssue(field=field_loc, issue_type=issue_type, description=f"Field '{field_loc}': {msg}", severity="error"))

        return ValidateSchemaResponse(is_valid=False, resource_type=res_type, issues=issues, validated_resource=None)
    except Exception as e:
        logger.error(f"Unexpected error during FHIR validation: {e}")
        issues.append(ValidationIssue(field="root", issue_type="syntax_error", description=f"Malformed FHIR payload: {e!s}", severity="error"))
        return ValidateSchemaResponse(is_valid=False, resource_type=res_type, issues=issues)
