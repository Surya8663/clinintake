...
app = FastAPI(title=settings.service_name, description="Workflow Orchestrator central hub", version="2.0.0", lifespan=lifespan)
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    err_envelope = ApiErrorEnvelope(code="INTERNAL_SERVER_ERROR", message="An unexpected server error occurred. Clinical workflow state preserved.", retryable=True, dependency="workflow-orchestrator")
...
async def create_document(req: CreateDocumentRequest, claims: dict[str, Any] = Depends(get_current_user_claims)):
    existing = await persistence.get_workflow(req.document_id)
    if existing:
        raise HTTPException(status_code=400, detail="Document already exists")

    workflow = DocumentWorkflow(document_id=req.document_id, state="received", context={"file_path": req.file_path})
    await persistence.save_workflow(workflow)

    await audit_event_bus.publish_event(event_type="document_received", document_id=req.document_id, payload={"file_path": req.file_path})
...
async def get_document(document_id: str, claims: dict[str, Any] = Depends(get_current_user_claims)):
...
async def transition_document(document_id: str, req: TransitionRequest, claims: dict[str, Any] = Depends(get_current_user_claims)):
    workflow = await persistence.get_workflow(document_id)
    if not workflow:
        raise HTTPException(status_code=404, detail="Document workflow not found")
    ...
    try:
        ...
        await audit_event_bus.publish_event(event_type=f"workflow_transition:{req.trigger}", document_id=document_id, payload={"new_state": workflow.state, "context": workflow.context})
...
async def execute_step(document_id: str, claims: dict[str, Any] = Depends(get_current_user_claims)):
    """
    ...
    try:
        lyzr_result = lyzr_client.start_superflow_execution(
            workflow_id=settings.lyzr_superflow_id, document_id=document_id, input_payload={"file_path": workflow.context.get("file_path", ""), "state": workflow.state}
        )
    ...
    if current_state == "received":
        scan_req = FilterScanRequest(document_id=document_id, file_path=workflow.context.get("file_path", ""))
        try:
            workflow = transition_workflow(workflow, "start_sanitize")
            await persistence.save_workflow(workflow)

            resp = await dispatch_downstream_call(service_name="document-security-filter", url=f"{settings.document_security_filter_url}/filter/scan", payload=scan_req)
    ...
    # 2. State: extracting -> Dispatch to /extract
        ...
        extract_req = ExtractRequest(document_id=document_id, file_path=workflow.context.get("clean_file_path", workflow.context.get("file_path", "")))
        try:
            resp = await dispatch_downstream_call(service_name="extraction-agent", url=f"{settings.extraction_agent_url}/extract", payload=extract_req)
            extract_resp = ExtractResponse(**resp)
            workflow.context["extracted_data"] = {"medications": extract_resp.medications, "diagnoses": extract_resp.diagnoses, "labs": extract_resp.labs}
    ...
    # 3. State: validating -> Dispatch to /validate/schema
        ...
        validate_req = SchemaValidateRequest(document_id=document_id, clinical_data=workflow.context["extracted_data"])
        try:
            resp = await dispatch_downstream_call(service_name="schema-validator", url=f"{settings.schema_validator_url}/validate/schema", payload=validate_req)
    ...
    # 4. State: reasoning -> Orchestrate care gaps pipeline (CQL -> Temporal -> Interactions -> Guideline -> Explanation -> Draft -> Guardrail)
        ...
        try:
            ...
            explain_resp = await dispatch_downstream_call(
                ...
                payload=CareGapExplainRequest(document_id=document_id, patient_id=patient_id, raw_care_gaps=cql_resp.get("care_gaps_identified", []), guideline_evidence=[]),
            )
    ...
    # 5. State: writing_ehr -> Dispatch to /fhir/write-transaction
        ...
        ehr_req = FhirWriteTransactionRequest(
            document_id=document_id, patient_id=target_patient_id, idempotency_key=f"IDEM-WRITE-{document_id}", fhir_resources=[{"resourceType": "Patient", "id": target_patient_id}]
        )
        try:
            resp = await dispatch_downstream_call(service_name="fhir-integration-service", url=f"{settings.fhir_integration_service_url}/fhir/write-transaction", payload=ehr_req)
        ...
        raise HTTPException(status_code=400, detail=f"No automatic transition defined for active state: {current_state}")
